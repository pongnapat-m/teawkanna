...
if ($promotion_id > 0) {
    ...
    $used = $conn->prepare(
        "SELECT usage_id FROM promotion_usage WHERE user_id=? AND promotion_id=? LIMIT 1"
    );
...
try {
    ...
    if ($promotion_id > 0) {
        ...
        $usage->bind_param('iiid', $_SESSION['user_id'], $promotion_id, $booking_id, $discount_amount);
        if (!$usage->execute()) {
