...
file_put_contents(__DIR__ . '/debug.log', date('Y-m-d H:i:s') . " - File validation passed\n", FILE_APPEND);
...
if (!is_dir(SLIP_UPLOAD_DIR)) {
    if (!mkdir(SLIP_UPLOAD_DIR, 0755, true)) {
        echo json_encode(['status' => 'error', 'message' => 'ไม่สามารถสร้างโฟลเดอร์ uploads/slips/ ได้ กรุณาสร้างด้วยตนเองและตั้ง permission 755']);
        exit;
    }
}
...
// ⚠️ ต้องมี 'Rejected' ด้วย ไม่งั้น ALTER TABLE จะลบค่า Rejected ออกจาก enum
// ทำให้ booking ที่ถูก reject แล้วกลับมาเป็น '' และถูก migrate เป็น PendingReview อีก
...
// ── Update booking status → PendingReview (ให้ user เห็น "รอการตรวจสอบ") ────
...
// ── Fix existing bookings stuck with empty status ('' due to old enum) ───────
...
file_put_contents(__DIR__ . '/debug.log', date('Y-m-d H:i:s') . " - Payment updated\n", FILE_APPEND);
...
/**
 * ส่ง SMS ผ่าน Twilio REST API (ไม่ต้องใช้ SDK)
 */
function sendTwilioSMS(string $to, string $body): bool {
    ...
    return isset($resp['sid']); // SID หมายถึงส่งสำเร็จ
}
